// Description:
//   Is Hubot is thinking the same thing you are?
//
// Dependencies:
//   None
//
...
//
// Commands:
//   hubot are you pondering what I'm pondering?
//
// http://pinkyandthebrain.wikia.com/wiki/Are_You_Pondering_What_I'm_Pondering%3F
/* eslint-disable */
const RESPONSES = [
...
/* eslint-enable */
// NOTE: we can't export this to unit test because of hubot's design:
//     WARNING Expected ./scripts/pondering to assign a function to module.exports, got object
function _changeNames(text, brainName) {
  return text.replace(/Brain\w*/, brainName);
}
module.exports = function (robot) {
  robot.respond(/.*(pondering|thinking).*\?$/i, (msg) => {
    const brainName = msg.message.user.name;
    const rawResponse = RESPONSES[Math.floor(Math.random() * RESPONSES.length)];
    msg.send(_changeNames(rawResponse, brainName));
  });
};
